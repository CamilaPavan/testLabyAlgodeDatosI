...
organizadores = {
    ...
    "AM": "amigos",
    "PR": "propio"
    } #Diccionario, para mostrar las claves "for i in organizadores "
lista_eventos = [] #lista vacia
class GestorEvento: #no usa contructor porque no se agregan atributos
    def instanciar_eventos(self): #es un metodo de una clase
        while True:
            ...
            if(instancia == "1" or instancia == "2" or instancia == "3"):
                break #se rompe para que siga la funcion
            else:
                print("Opcion incorrecta!")
        #Nombre del evento, verificar
        existe = True #FLAG
        while True:
            nombre_evento = input("ingrese el nombre: ")
            for i in lista_eventos: #recorrer la lista y buscar si existe.
                if(nombre_evento == i.get_nombre()):
                    print("ese nombre ya existe!")
                    existe = False
                    break
            if(existe): #el nombre es nuevo.
                break
        #verificar la fecha
        while True:
        ...
        hora = input("ingrese la hora: ") #la pedimos sin verificar porque no hace falta
        lugar = input("ingrese el lugar: ")
        
        if(instancia =="1"):
            evento = cl.Evento(nombre_evento,fecha,hora,lugar) #para usar las clases
